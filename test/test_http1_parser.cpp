...
TEST_CASE("HTTP/1.1 request-line parsing") {
    ...
    auto result = netpipe::http1::parse_request_line("GET /status HTTP/1.1", request);
    CHECK(result.is_ok());
...
TEST_CASE("HTTP/1.1 status-line parsing") {
    ...
    auto result = netpipe::http1::parse_status_line("HTTP/1.1 204 No Content", response);
    CHECK(result.is_ok());
...
TEST_CASE("HTTP/1.1 header parsing and lookup") {
    auto header = netpipe::http1::parse_header_line("Content-Type: application/json");
    CHECK(header.is_ok());
    ...
    auto found = netpipe::http1::find_header_value(headers, "content-type");
    CHECK(found.has_value());
    CHECK(found.value() == "application/json");
}

TEST_CASE("HTTP/1.1 request head parser rejects obs-fold") {
    auto parsed = netpipe::http1::parse_request_head("GET / HTTP/1.1\r\n"
                                                      "Host: example.com\r\n"
                                                      " User-Agent: bad-fold\r\n"
                                                      "\r\n");
    CHECK(parsed.is_err());
...
TEST_CASE("HTTP/1.1 parse full request and response heads") {
    auto request = netpipe::http1::parse_request_head("POST /v1/items HTTP/1.1\r\n"
    ...
    auto response = netpipe::http1::parse_response_head("HTTP/1.1 200 OK\r\n"
                                                         "Content-Length: 0\r\n"
