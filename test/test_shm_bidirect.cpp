...
TEST_CASE("ShmStream + Remote<Bidirect> - 10MB payload") {
    ...
    std::thread server_thread([&]() {
        ...
        // Wait for client call to complete first (sequential, not concurrent)
        while (finished_count < 1) {
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
        }

        // Small delay to ensure client's response is fully processed
        ...
        // Signal server call is done
        finished_count++;

        // Wait for client to also finish
    ...
    std::thread client_thread([&]() {
        ...
        // Signal client call is done so server can proceed
        finished_count++;
        ...
        // Signal we're completely done
        finished_count++;

        // Wait for server to also finish
        while (finished_count < 3) {
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
        }
...
TEST_CASE("ShmStream + Remote<Bidirect> - 100MB payload") {
    netpipe::ShmStream server_stream;
    // Buffer needs to hold: 100MB request + 100MB response + protocol overhead
    // Use 1GB total = 512MB per direction (plenty of room for 100MB message + overhead)
    ...
    std::thread server_thread([&]() {
        ...
        // Longer delay to ensure client's response is fully processed and buffer is drained
...
TEST_CASE("ShmStream + Remote<Bidirect> - 1GB payload") {
    netpipe::ShmStream server_stream;
    // Buffer needs to hold: 1GB request + 1GB response + protocol overhead
    // Use 3GB total = 1.5GB per direction (enough for 1GB message + overhead)
    ...
    std::thread server_thread([&]() {
        ...
        auto server_resp = server_remote.call(2, server_request, 120000); // 2 minute timeout for 1GB
        REQUIRE(server_resp.is_ok());
    ...
    std::thread client_thread([&]() {
        ...
        auto client_resp = client_remote.call(1, client_request, 120000); // 2 minute timeout for 1GB
        REQUIRE(client_resp.is_ok());
