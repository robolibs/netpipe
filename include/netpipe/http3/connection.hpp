...
#include <netpipe/http3/frame.hpp>
#include <netpipe/http3/qpack.hpp>
        ...
        dp::Res<void> process_control_data(const dp::Vector<dp::u8> &data) {
            ...
            while (offset < data.size()) {
                ...
                case FrameType::Settings: {
                    if (settings_received_) {
                        return dp::result::err(
                            dp::Error::invalid_argument("duplicate SETTINGS frame"));
                    }

                    auto settings_result =
                        SettingsFrame::parse(data.data() + offset, data.size() - offset);
                ...
                case FrameType::GoAway: {
                    auto goaway_result =
                        GoAwayFrame::parse(data.data() + offset, data.size() - offset);
                ...
                default:
                    ...
                    auto len_result =
                        quic::varint_decode(data.data() + offset, data.size() - offset);
        ...
        dp::Res<void> process_request_stream(dp::u64 stream_id, const dp::Vector<dp::u8> &data) {
            ...
            while (offset < data.size()) {
                ...
                case FrameType::Headers: {
                    auto headers_result =
                        HeadersFrame::parse(data.data() + offset, data.size() - offset);
                ...
                case FrameType::Data: {
                    ...
                    // Append to body
                    if (is_client_) {
                        it->second.response.body.insert(it->second.response.body.end(),
                                                        frame.data.begin(), frame.data.end());
                    } else {
                        it->second.request.body.insert(it->second.request.body.end(),
                                                       frame.data.begin(), frame.data.end());
                    }
                    break;
                }
