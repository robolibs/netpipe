/// Example: IPC + Remote<Unidirect>
/// Simple client-server RPC over Unix domain sockets
...
void test_payload(dp::usize size_mb) {
    ...
    std::thread server_thread([size_mb]() {
        ...
        netpipe::IpcEndpoint endpoint{"/tmp/netpipe_ipc_unidirect.sock"};
        server.listen_ipc(endpoint);
